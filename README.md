...
### 7. Get edit event form data

- [ ] Clicking the 'Edit Event' button from the schedule page will send a GET request to `/events/:id` load the data for that event and display it in the form. Look at the `GET /events/:id` route in `routes/events.ts`.
- [ ] Build a `getEventById` function with an `id` parameter. Use this in your route.
 <details style="padding-left: 2em">
  <summary>More about completing the route</summary>
  
  The GET route should return either the data for the individual event from the database, or a 404 error if the event is not in the database.

  - Implement a conditional to check for the result of your database call. If it is undefined, you will need to make use of a `.sendStatus` call with the correct status code.
  - Your route should only send one response in total. Check that the if statement has a `return` statement so that execution ends after the `.sendStatus` call.
  </details>
<br>
...
This is all the functionality you need to complete the assessment for this challenge. However, there are some further steps that you can complete to prepare it for the software quality assessment.
...
<details style="padding-left: 2em">
  ...
  - Testing
    ...
    - Complete the backend `server/__tests__/DeletingAnEvent.test.ts` test file to test delete functionality for an event
      - To complete the test, you will need to have completed the GET route from Step 7 including the 404 response

  Check out the CP02 Assessment ticket for more information
</details>
<br>
